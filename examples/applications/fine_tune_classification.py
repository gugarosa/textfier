...
X_train, Y_train = loader.load_csv("data/csv/train.csv")
X_test, Y_test = loader.load_csv("data/csv/test.csv")

# Creates the sequence classification task with pre-trained model
task = SequenceClassificationTask(
    model="neuralmind/bert-base-portuguese-cased", num_labels=2
)

# Encodes the training and testing samples
X_enc_train = task.tokenizer(
    X_train, return_tensors="pt", padding=True, truncation=True
)
X_enc_test = task.tokenizer(X_test, return_tensors="pt", padding=True, truncation=True)

# Creates the datasets
train_dataset = Dataset(
    input_ids=X_enc_train["input_ids"],
    attention_mask=X_enc_train["attention_mask"],
    labels=Y_train,
)
test_dataset = Dataset(
    input_ids=X_enc_test["input_ids"],
    attention_mask=X_enc_test["attention_mask"],
    labels=Y_test,
)
